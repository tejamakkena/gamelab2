...
def register_trivia_events(socketio):
    ...
    def handle_leave_room(data):
        ...
        for player_name, player_data in room['players'].items():
            if player_data.get('sid') == (session.sid if hasattr(session, 'sid') else None):
    ...
    def handle_start_game(data):
        ...
        # For now, using a placeholder - you should integrate with your generate route
        ...
        try:
            ...
            emit('error', {'message': f'Failed to generate questions: {str(e)}'}, room=room_code)
            room['status'] = 'waiting'
