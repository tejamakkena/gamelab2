...
def register_snake_events(socketio):
    ...
    def handle_join_room(data):
        ...
        print(f"🎮 {player_name} (ID: {player_id}) trying to join room {room_code}...")
        ...
        print(f"📊 Room now has {len(room['players'])} players: {[p['name'] for p in room['players']]}")
        ...
        # Notify ALL players in the room (including host) with updated player list
        ...
        print(f"📤 Broadcasted player join to all {len(room['players'])} players in room {room_code}")
    ...
    def handle_leave_room(data):
        ...
        if room['host'] == player_id and len(room['players']) > 0:
            ...
            print(f"👑 New host assigned in room {room_code}: {room['players'][0]['name']}")
    ...
    def handle_start_game(data):
        ...
        print(f"🎮 Start game request for room {room_code} by player {player_id}")
        ...
        if room['host'] != player_id:
            print(f"❌ Player {player_id} is not the host. Host is {room['host']}")
        ...
        if len(room['players']) < 2:
            ...
            emit('snake_error', {'message': 'Need at least 2 players to start!'})
            return
        ...
        print(f"🎮 Game started in room {room_code} with {len(room['players'])} players")
    ...
    def handle_roll_dice(data):
        ...
        if new_position in SNAKES:
            snake_or_ladder = {'type': 'snake', 'from': new_position, 'to': SNAKES[new_position]}
            new_position = SNAKES[new_position]
        elif new_position in LADDERS:
            snake_or_ladder = {'type': 'ladder', 'from': new_position, 'to': LADDERS[new_position]}
        ...
        if new_position == 100:
            ...
            cleanup_thread = threading.Thread(target=cleanup_room, args=(socketio, room_code))
    ...
    def handle_disconnect():
        ...
        for room_code, room in list(snake_rooms.items()):
            player = next((p for p in room['players'] if p['id'] == player_id), None)
            if player:
                print(f"🔌 Player {player['name']} disconnected from room {room_code}")
